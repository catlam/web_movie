...
function Filters(props) {
    ...
    return (
        <div className="my-6 bbg-dry border text-dryGray border-gray-800 grid md:grid-cols-5 grid-cols-2 lg:gap-12 gap-2 rounded p-6">
            {
                                ...
                                <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full bg-white border-gray-800 text-dryGray rounded-md shadow-lg max:h-60 py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                    {
                                        item.items.map((iterm, i) => (
                                            <ListboxOption key={i} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? "bg-subMain text-white" : "text-main"
                                                }`} value={iterm}>
                                                {({ selected }) => (
                                                    <>
